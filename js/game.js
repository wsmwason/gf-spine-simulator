...
var game = {
	...
	init : function(){
		...
		game.renderer = new PIXI.CanvasRenderer(600, 800, {transparent : true});
	...
	start : function(){
        ...
        game.player.spine[game.player.name].x = 300;
        game.player.spine[game.player.name].y = 560;
        game.player.spine[game.player.name].scale.x = 1.0;
        game.player.spine[game.player.name].scale.y = 1.0;
		$('#container').html(game.renderer.view);
	},
	animate : function(){
		game.animationFrame = window.requestAnimationFrame(game.animate, game.renderer);
